...
class BankConnectionUpdateTaskDetails implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    protected static array $openAPINullables = [
		...
		'import_new_accounts_mode' => true,
		'account_types' => true,
		'skip_balances_download' => true,
    ...
    protected static $attributeMap = [
        ...
        'import_new_accounts_mode' => 'importNewAccountsMode',
    ...
    protected static $setters = [
        ...
        'import_new_accounts_mode' => 'setImportNewAccountsMode',
    ...
    protected static $getters = [
        ...
        'import_new_accounts_mode' => 'getImportNewAccountsMode',
    ...
    public const IMPORT_NEW_ACCOUNTS_MODE_SKIP = 'SKIP';
    public const IMPORT_NEW_ACCOUNTS_MODE_CONDITIONAL = 'CONDITIONAL';
    public const IMPORT_NEW_ACCOUNTS_MODE_FORCED = 'FORCED';
     ...
     * @return string[]
     */
    public function getImportNewAccountsModeAllowableValues()
    {
        return [
            self::IMPORT_NEW_ACCOUNTS_MODE_SKIP,
            self::IMPORT_NEW_ACCOUNTS_MODE_CONDITIONAL,
            self::IMPORT_NEW_ACCOUNTS_MODE_FORCED,
        ];
    }
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->setIfExists('import_new_accounts_mode', $data ?? [], 'SKIP');
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if ($this->container['import_new_accounts_mode'] === null) {
            $invalidProperties[] = "'import_new_accounts_mode' can't be null";
        }
        $allowedValues = $this->getImportNewAccountsModeAllowableValues();
        if (!is_null($this->container['import_new_accounts_mode']) && !in_array($this->container['import_new_accounts_mode'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'import_new_accounts_mode', must be one of '%s'",
                $this->container['import_new_accounts_mode'],
    ...
    /**
     * Gets import_new_accounts_mode
     *
     * @return string
     */
    public function getImportNewAccountsMode()
    {
        return $this->container['import_new_accounts_mode'];
    }

    /**
     * Sets import_new_accounts_mode
     *
     * @param string $import_new_accounts_mode This parameter defines how we should deal with new accounts that the user might have at the bank but which have not gotten imported yet.<br/>The user will be able to stop the process once he finds his relevant accounts imported. The set of newly imported accounts can get limited also by using the \"accountTypes\" parameter.<br/><br/><strong>Import new accounts mode:</strong><br/>&bull; <code>SKIP</code> - no new accounts will get imported;<br/>&bull; <code>CONDITIONAL</code> -  For the <code>XS2A</code> interface new accounts will get imported only if the stored consent supports this capability (see: <code>consent.supportsImportNewAccounts = true</code> in Access). For the other interfaces, new accounts will always get imported.<br/>&bull; <code>FORCED</code> - For the <code>XS2A</code> interface, the stored consent will get deleted before the update is triggered, in case it doesn't support importing new accounts. For the other interfaces, new accounts will always get imported.<br/><br/><strong>NOTE:</strong> No matter the option chosen, a web form can still be generated in the background.
     *
     * @return self
     */
    public function setImportNewAccountsMode($import_new_accounts_mode)
    {
        if (is_null($import_new_accounts_mode)) {
            array_push($this->openAPINullablesSetToNull, 'import_new_accounts_mode');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('import_new_accounts_mode', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
        ...
        $allowedValues = $this->getImportNewAccountsModeAllowableValues();
        if (!is_null($import_new_accounts_mode) && !in_array($import_new_accounts_mode, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value '%s' for 'import_new_accounts_mode', must be one of '%s'",
                    $import_new_accounts_mode,
                    implode("', '", $allowedValues)
                )
            );
        }
        $this->container['import_new_accounts_mode'] = $import_new_accounts_mode;
    ...
    public function setSkipBalancesDownload($skip_balances_download)
    {
        if (is_null($skip_balances_download)) {
            array_push($this->openAPINullablesSetToNull, 'skip_balances_download');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('skip_balances_download', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
